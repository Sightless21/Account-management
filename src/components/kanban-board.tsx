...
const Column = ({
  ...
  const handleDragEnd = (e: DragEvent) => {
    ...
    console.log(
      "🚀 ~ file: Columns.tsx:33 ~ handleDragEnd ~ cardId : ",
      cardId,
    );
  ...
  return (
    <div
      className={`py-x-2 h-[500px] w-72 shrink-0 rounded border-4 ${headingBgColor}`}
    >
      <div
        className={`mb-3 flex items-center justify-between overflow-y-auto p-2`}
      >
      ...
      <div
        ...
        className={`h-[435px] w-full snap-x border-t-4 border-dotted border-black p-4 transition-colors ease-in-out ${active ? "bg-neutral-800/20" : "bg-neutral-800/0"} overflow-y-auto`}
      >
        {filteredCards.map((c) => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="flex cursor-grab snap-center flex-col items-start justify-between rounded border border-neutral-700 bg-white hover:bg-neutral-300 active:cursor-grabbing"
        whileTap={{ scale: 1.04 }}
      ...
      >
        ...
        <div className="mb-2 flex w-full items-center justify-between gap-x-9 bg-neutral-800 p-2">
          ...
          <div className="flex items-start gap-2">
            <p className="overflow-hidden text-sm text-neutral-100">{title}</p>
          </div>
          {/* Drag indicator */}
